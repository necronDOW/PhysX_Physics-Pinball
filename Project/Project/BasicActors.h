...
namespace PhysicsEngine
{
	...
	{
	public:
		Compound(const PxTransform& pose = PxTransform(PxIdentity), float radius = 1.f, int edgeCount = 3, PxVec2 thickness = PxVec2(.1f, .1f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
			float angleRad = (PxPi * 2.f) / (float)edgeCount;
			float len = SideLength(radius, edgeCount) / (edgeCount + 2.f);
			PxVec3 dimensions = PxVec3(len, thickness.y, thickness.x);
			PxVec3 tracer = PxVec3(0.f, radius / (2 * tan(PxPi / (float)edgeCount)), 0.f);

			for (int i = 0; i < edgeCount; i++)
			{
				CreateShape(PxBoxGeometry(dimensions), density);
				GetShape(i)->setLocalPose(PxTransform(tracer, PxQuat(angleRad * i, PxVec3(0, 0, 1))));

				tracer = Rotate(tracer, angleRad);
			}
		}
	...
	private:
		...
		PxReal SideLength(float apothem, int edgeCount)
		{
			return (apothem * 2) / sin(PxPi / (float)edgeCount);
		}
	};
